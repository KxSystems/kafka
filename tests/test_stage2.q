...
.test.ASSERT_EQ["get earliest offset since 1 minute ago for topic1"; last exec offset from .kafka.getEarliestOffsetsForTimes[consumer; `topic1; enlist[1i]!enlist .z.p-0D00:01:00.000000000; 1000]; current_offset_1];
.test.ASSERT_EQ["get earliest offset since 1 minute ago for topic2"; last exec offset from .kafka.getEarliestOffsetsForTimes[consumer; `topic2; enlist[1i]!enlist .z.p-0D00:01:00.000000000; 1000]; current_offset_2];

.test.ASSERT_EQ["offset for topic1 after publish"; (last exec offset from .kafka.getCommittedOffsetsForTopicPartition[consumer; `topic1; enlist 1i]) - current_offset_1; 2]
.test.ASSERT_EQ["offset for topic2 after publish"; (last exec offset from .kafka.getCommittedOffsetsForTopicPartition[consumer; `topic2; enlist 1i]) - current_offset_2; 1]
...
.test.ASSERT_EQ["offset proceeds for topic 2 after batch publish"; (last exec offset from .kafka.getCommittedOffsetsForTopicPartition[consumer; `topic2; enlist 1i]) - current_offset_2; 2]
